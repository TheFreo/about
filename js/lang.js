let arrLang = {
    'ru': {
        'menu': 'Меню',
        'main': 'Главная',
        'about': 'Обо мне',
        'welup': 'Я TheFreo,',
        'welun': ' Я начинающий программист. Люблю сидеть дома и смотреть аниме :3 Сейчас изучаю ReactJS и TypeScript.',
        'bio': 'Кто я?',
        'skills': 'Навыки',
        'devices': 'Девайсы',
        'pc': 'Основной ПК',
        'lap': 'Ноутбук (MateBook D14)',
        'os': 'Система:',
        'cpu': 'Процессор:',
        'gpu': 'Видеокарта:',
        'ram': 'ОЗУ:',
        'slap': 'Домашний сервер (Mac Mini 2011)',
        'name': 'Имя: Артём',
        'brith': 'День рождения: 15.06.2005 (Осталось ',
        'brith2': ' дней)',
        'country': 'Страна: Россия (мечтаю переехать в Японию или Европу)',
        'langu': 'Языки: Русский, Английский (30%), Украинский (70%)',
        'listen': 'Слушаю: Рок (Любой), IDM, Ambient',
        'cont': 'Контакты',
        'morning': 'Доброе утро!',
        'afternoon': 'Добрый день!',
        'evening': 'Добрый вечер!',
        'night': 'Доброй ночи!',
        'dev': '©2025, Разработано ',
        'age': 'Возраст: ',
        'today': 'Сегодня!'
    },
    'en': {
        'menu': 'Menu',
        'main': 'Main',
        'about': 'About me',
        'welup': "I'm TheFreo,",
        'welun': "I'm an aspiring programmer. I like to sit at home and watch anime :3 At the moment I am studying ReactJS and TypeScript.",
        'bio': 'Who Am I?',
        'skills': 'Skills',
        'devices': 'Devices',
        'pc': 'Main PC',
        'lap': 'Laptop (MateBook D14)',
        'slap': 'Home server (Mac Mini 2011)',
        'os': 'OS:',
        'cpu': 'CPU:',
        'gpu': 'GPU:',
        'ram': 'RAM:',
        'name': 'Name: Artem',
        'brith': 'Brithday: 15.06.2005 (through ',
        'brith2': ' days)',
        'country': 'Counrty: Russia (dream: Japan or Europe)',
        'langu': 'Languages: Russian, English (30%), Ukrainian (70%)',
        'listen': 'Listening: Rock (Any), IDM, Ambient',
        'cont': 'Contacts',
        'morning': 'Good morning!',
        'afternoon': 'Good afternoon!',
        'evening': 'Good evening!',
        'night': 'Good night!',
        'dev': '©2025, Crafted by ',
        'age': 'Age:',
        'today': 'Today!'
    }
};
